...
SoPDF 通过并行处理等优化技巧，性能明显优于同类产品：渲染速度最高快 **2.78 倍**，纯文本提取快 **2.7 倍**，全文搜索快 **3 倍**——且与 PyMuPDF 保持 99% 的词级准确率一致性。详见 [性能基准](#性能基准) 章节，或亲自运行验证。
...
| 渲染 1 页 @ 72 DPI | 4.1 ms | 9.0 ms | **快 2.20 倍** |
| 渲染 1 页 @ 150 DPI | 11.7 ms | 30.2 ms | **快 2.58 倍** |
| 渲染 1 页 @ 300 DPI | 34.2 ms | 95.2 ms | **快 2.78 倍** |
| 50 页顺序渲染 @ 150 DPI | 543 ms | 1468 ms | **快 2.70 倍** |
| 50 页并行渲染 @ 150 DPI | 418 ms | 446 ms | **快 1.07 倍** |

SoPDF 在每个 DPI 档位均胜出，且分辨率越高优势越明显。在多进程并行模式下，SoPDF 相较自身顺序基准实现了 **1.30 倍的加速**（差距收窄是因为顺序渲染本身已大幅提速）。PyMuPDF 的多线程并行路径则适得其反——因 MuPDF 全局锁将并发渲染串行化，耗时反而退化至 1510 ms，比顺序执行还慢。
